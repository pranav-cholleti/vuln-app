...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            # In the original code, the password was stored in plain text.
            # Now, we assume the password in the database is already hashed.
            ...
            if stored_user:
                stored_password_hash = stored_user[0]
...
def get_user(username):
    ...
    if user:
        # Returning password in the response - sensitive data exposure - FIXED
        return jsonify({
            'id': user[0],
            'username': user[1],
            # 'password': user[2],  # Sensitive data exposure! Removed.
            'role': user[3]
        })
    else:
...
def register():
    if request.method == 'POST':
        ...
        # Weak hashing algorithm - security misconfiguration - FIXED
